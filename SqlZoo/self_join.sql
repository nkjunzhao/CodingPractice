...
a count of 2. Add a HAVING clause to restrict the output to these two routes.*/
...
the services between 'Craiglockhart' and 'London Road' are shown. If you are tired of 
...
SELECT DISTINCT stops.name, company, num
  ...
  JOIN stops
    ON id = stop
  WHERE num IN (
    SELECT num from route
      JOIN stops
      ON id = stop
      WHERE stops.name = 'Craiglockhart' AND company = 'LRT') AND company = 'LRT';
...
SELECT DISTINCT a.num, a.company, name, c.num, c.company
  FROM route a
  JOIN route b
    ON a.num = b.num and a.company=b.company
  JOIN route c
  ...
  WHERE a.stop = (
    SELECT id
      FROM stops
      WHERE name = 'Craiglockhart') AND d.stop = (
    SELECT id
      FROM stops
